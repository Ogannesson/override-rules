...
(() => {
  ...
  // src/constants.ts
  var NODE_SUFFIX, CDN_URL, SPEEDTEST_URL, LOW_COST_NODE_MATCHER, WARP_NODE_MATCHER, PROXY_GROUPS, countriesMeta;
  var init_constants = __esm({
    "src/constants.ts"() {
      ...
      WARP_NODE_MATCHER = createCaseInsensitiveNodeMatcher(String.raw`warp|cloudflare`);
      PROXY_GROUPS = {
        SELECT: "选择代理",
        MANUAL: "手动选择",
        AUTO: "自动选择",
  ...
  function buildProxyGroups({
    ...
    const groups = [
      ...
      lowCostNodes.length > 0 || regexFilter ? buildGroupByType({
        ...
        groupType,
        nodeSource: !regexFilter ? { proxies: lowCostNodes.map((node) => node.name).filter(isNotNull) } : {
          "include-all": true,
      ...
      warpNodes.length > 0 || regexFilter ? {
        ...
        type: "select",
        ...!regexFilter ? { proxies: warpNodes.map((node) => node.name).filter(isNotNull) } : { "include-all": true, filter: WARP_NODE_MATCHER.pattern }
      } : null,
      ...countryNames.map((country) => {
        ...
        const nodeSource = regexFilter ? {
          "include-all": true,
          filter: meta.pattern,
          "exclude-filter": meta.excludePattern ? `${meta.excludePattern}|${WARP_EXCLUDE}` : WARP_EXCLUDE
